...
# Custom Master Executive Dark Theme
st.markdown("""
<style>
...
if file_dm and files_sl:
    ...
    # ---------------------------------------------------------
    # TAB 2: PHÂN BỔ THỊ TRƯỜNG & TỶ TRỌNG SẢN PHẨM
    # ---------------------------------------------------------
    with tab2:
        st.markdown("##### 🗺️ Biểu Đồ Cây Phân Cấp Quy Mô Tổng Thị Trường (Treemap)")
        st.caption("💡 *Quy mô ô vuông thể hiện độ lớn của doanh số/sản lượng.*")
        ...
        if not tree_df.empty:
            ...
            fig_tree.update_layout(**DARK_LAYOUT, height=450, coloraxis_showscale=False)
            st.plotly_chart(fig_tree, use_container_width=True)

        st.markdown("---")
        st.markdown("##### 🎯 Cơ Cấu Thị Trường Tiêu Thụ Trọng Yếu Theo Từng Sản Phẩm (%)")
        st.caption("💡 *Biểu đồ cột chồng 100% giúp nhận diện ngay thị trường (địa bàn) nào là 'nồi cơm' chính của từng SKU.*")

        prod_reg_df = df_filtered.groupby(['Ten_SP', 'Dia_Ban_Tieu_Thu'])[unit_choice].sum().reset_index()
        prod_total = prod_reg_df.groupby('Ten_SP')[unit_choice].transform('sum')
        prod_reg_df['Phan_Tram'] = (prod_reg_df[unit_choice] / prod_total) * 100
        prod_reg_df = prod_reg_df[prod_reg_df[unit_choice] > 0].sort_values(by=['Ten_SP', 'Phan_Tram'], ascending=[True, False])

        if not prod_reg_df.empty:
            fig_stacked = px.bar(
                prod_reg_df, 
                y='Ten_SP', 
                x='Phan_Tram', 
                color='Dia_Ban_Tieu_Thu',
                orientation='h',
                color_discrete_sequence=EXECUTIVE_PALETTE,
                text=prod_reg_df['Phan_Tram'].apply(lambda x: f"{x:.1f}%" if x > 6 else ""),
                hover_name='Dia_Ban_Tieu_Thu',
                hover_data={'Dia_Ban_Tieu_Thu': False, 'Ten_SP': False, 'Phan_Tram': ':.1f', unit_choice: True}
            )
            fig_stacked.update_traces(
                textposition='inside', 
                insidetextanchor='middle',
                marker_line=dict(width=1, color='rgba(255,255,255,0.2)'),
                textfont=dict(color='#ffffff', size=11, family="Plus Jakarta Sans", weight="bold")
            )
            layout_stacked = DARK_LAYOUT.copy()
            layout_stacked['margin'] = dict(l=20, r=20, t=30, b=40)
            fig_stacked.update_layout(
                **layout_stacked,
                height=480,
                barmode='stack',
                xaxis=dict(showgrid=True, gridcolor='rgba(255,255,255,0.06)', title="Tỷ trọng Đóng góp (%)", ticksuffix="%"),
                yaxis=dict(title=""),
                legend=dict(orientation="h", yanchor="bottom", y=1.05, xanchor="left", x=0, title_text="Phân Lớp Địa Bàn:")
            )
            st.plotly_chart(fig_stacked, use_container_width=True)

    # ---------------------------------------------------------
    # TAB 3: XU HƯỚNG TĂNG TRƯỜNG
    # ---------------------------------------------------------
    with tab3:
        ...
        if not trend_df.empty:
            ...
            fig_line.update_layout(
                **layout_trend, 
                ...
                xaxis_title="",
                yaxis=dict(title=y_label, showgrid=True, gridcolor='rgba(255,255,255,0.06)', tickformat=".1f")
            )
            st.plotly_chart(fig_line, use_container_width=True)

    # ---------------------------------------------------------
    # TAB 4: NĂNG LỰC & RỦI RO NPP
    # ---------------------------------------------------------
    with tab4:
